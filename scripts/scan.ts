...
async function main() {
  ...
  const r = scan({ chain, gamma: g, budget, minDte: 5, maxDte: 60, bias: "auto" });
  console.log(`${chain.underlying.symbol} $${chain.underlying.price.toFixed(2)}  bias=${r.bias} target=$${r.target.toFixed(2)} regime=${g.regime} considered=${r.considered}`);
